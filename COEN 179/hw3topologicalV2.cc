...
bool IsAncestor(GraphNode* p, GraphNode*c) {
	...
	cout<<"Parent: "<<p->id<<" --- Looking for child "<<c->id<<endl;
	while(child!=NULL){
	    cout<<"\tChild: "<<child->graph_node->id<<endl;
...
bool IsTopologicalOrder(GraphNode graph[], int order[]) {
	for(int i=NUMNODES-1;i>0;i--){
		for(int j=i-1;j>=0;j--){
			if(!IsAncestor(  &(graph[order[j]])  ,  &(graph[order[i]]))  ){
			    return false;
			}
		}
	}
...
void TopologicalSortBruteForce(GraphNode graph[]) {
    cout<<"TopologicalSortBruteForce running"<<endl;
...
int main() {
  ...
  if(IsAncestor(&graph[1],&graph[5])) cout<<"True"<<endl;
