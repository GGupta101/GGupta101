...
//Name: Gagan Gupta
//Date: 03/06/20
//Assignment: Lab 9 Binary Search Tree Bag Header and Implementation
...
#pragma mark - Toolkit Function Implementation
    ...
    {
		//Remove the max element of the tree (right most node) and replace removed with that nodes data 
    ...
    {
		//If less than go left, if more than go right, and if equal remove (if left NULL then delete else remove max of left subtree)
        binary_tree_node<Item> *oldroot_ptr;
        if (root_ptr == NULL){   
		// Empty tree
            return false;
    ...
    {
		//Remove all the instances of target by using bst_remove. Everytime you delete a target you return 1 plus the recurrsive call of remove_all on the root_ptr
        binary_tree_node<Item> *oldroot_ptr;
        if (root_ptr == NULL){   
		// Empty tree
            return 0;
...
#pragma mark - Member Function Implementation
    ...
    {
		//Copy constructor using tree_copy from bintree.h to copy source into this
    ...
    {
		//Destructor calling tree clear from bintree.h
        tree_clear(root_ptr);
    }
    ...
    // Header file used: bintree.h
    {
		//Return the size of the tree by using tree_size from bintree.h
    ...
    // Header file used: bintree.h
    {
		//Insert function for the BST. Keep going left and right (<= and > respectively) till you reach the value of null then insert
    ...
    typename bag<Item>::size_type bag<Item>::count(const Item& target) const{
		//Count the number of targets in the BST
        size_type answer = 0;
        binary_tree_node<Item> *cursor = root_ptr;
    ...
    typename bag<Item>::size_type bag<Item>::erase(const Item& target){
		//Erase calls the remove_all function from bintree.h to erase all instances of target
        return bst_remove_all(root_ptr, target);
    }
    
    template <class Item>
    bool bag<Item>::erase_one(const Item& target){
		//Erase calls the remove function from bintree.h to erase the first instance of target
    ...
    // Header file used: bintree.h
    {
		//Assignment operator calls tree_copy similarly to the copy constructor for BST
        if (this==&source){
			return;
		}
        tree_clear(root_ptr);
    ...
    void bag<Item>::operator +=(const bag<Item>& addend){
		//Call the insert_all function from the bintree.h file to implement the += operator for BST
    ...
    template <class Item>
    bag<Item> operator +(const bag<Item>& b1, const bag<Item>& b2){
		//Simply use the previous implemented operator += to implement the + operator
        bag<Item> temp=b1;
        temp+=b2;
        return temp;
    }
    ...
    // method.
    {
		//Recurrsively add all of the nodes of a BST in a preorder manor
        if (addroot_ptr != NULL){
