...
//Name: Gagan Gupta
//Date: 03/06/20
//Assignment: Lab 9 Binary Tree Header and Implementation
...
namespace coen79_lab9
{
    ...
    {
		//Inorder is left then middle then right
        if (node_ptr != NULL){
    ...
    // Library facilities used: cstdlib
    {
		//Postorder is left then right then middle
        if(node_ptr!=NULL){
            inorder(f, node_ptr->left());
            inorder(f, node_ptr->right());
            f(node_ptr->data());
    ...
    // Library facilities used: cstdlib
    {
		//Preorder is middle then left then right
        if(node_ptr!=NULL){
            f(node_ptr->data());
            inorder(f, node_ptr->left());
            inorder(f, node_ptr->right());
    ...
    {
		//Given function to print the Binary Tree
        if (node_ptr != NULL){
    ...
    // Library facilities used: cstdlib
    {
		//Clear the root and then clear the children of the root recurrsively
    ...
    // Library facilities used: cstdlib
    {
		//Copy the root and then copy the children of the root recurrsively. When reach past a leaf we return NULL
        ...
        if (root_ptr == NULL){
            ...
            binary_tree_node<Item>* treeCopy = new binary_tree_node<Item>(root_ptr->data(),l_ptr,r_ptr);
			return treeCopy;
        }
    }
    ...
    // Library facilities used: cstdlib
    {
		//Recurrsively count the amout of nodes in the tree
        if (node_ptr == NULL){
            return 0;
		}
        else{
